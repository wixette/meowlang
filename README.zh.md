...
| 10 | `YOWL` | 大声嗷叫 | **ASCII 输出**: 弹出列表尾部元素的值，并打印对应的 ASCII 字符 | `IP++` |
| 11 | `SNIFF` | 嗅探环境 | **ASCII 输入**: 从标准输入读取一个字符，将其 ASCII 码压入列表尾部 | `IP++` |
| 12 | `NAP` | 猫咪小睡 | **休眠/延迟**: 弹出列表尾部元素的值，并暂停执行对应的毫秒数 | `IP++` |
| 13 | `SCRATCH` | 清理领地 | **清屏**: 清除输出控制台或屏幕内容 | `IP++` |
| >=14 | `NOP` | - | 无操作 | `IP++` |
